...
extension Bundle {
   ...
   -> [UserModel] {
      ...
      guard let _url = self.url(forResource: file,
                                withExtension: nil)
      else { fatalError("Failed to locate \(file) in the bundle.") }
       ...
       */
      else { fatalError("Failed to load \(file) from the bundle.") }
      ...
      guard let _decodedData = try? jsonDecoder.decode([UserModel].self,
                                                       from: _data)
      else { fatalError("Failed to decode \(file) from the bundle.") }
